...
{
    public sealed class UpdateOrder_OrderStatusTypeReadyBehavior : IPipelineBehavior<UpdateOrder_OrderStatusType, bool>
    {
        #region Fields
        ...
        public UpdateOrder_OrderStatusTypeReadyBehavior(IMediator mediator)
        {
        ...
        public async Task<bool> Handle(UpdateOrder_OrderStatusType command, RequestHandlerDelegate<bool> next, CancellationToken cancellationToken)
        {
            ...
            if (command.NewOrderStatusType != OrderStatusType.READY)
            {
                return updateOrder_OrderStatusTypeResponse;
            }
